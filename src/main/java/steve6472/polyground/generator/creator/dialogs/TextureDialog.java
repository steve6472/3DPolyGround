...
public class TextureDialog extends OkDialog
{
	...
	public TextureDialog(Atlas atlas)
	{
...
//		selY = face.getProperty(FaceRegistry.texture).getTextureId() / atlas.getTileCount();
	...
	public void render()
	{
		...
		if (mx >= 0 && my >= 0 && mx < s && my < s)
		{
			SpriteRender.fillRect(sx + (mx >> 5) * 32, sy + (my >> 5) * 32, 32, 32, 0.8f, 0.8f, 0.8f, 0.5f);
		}

		if (res != -1)
		{
			SpriteRender.fillRect(sx + selX * 32, sy + selY * 32, 32, 32, 0.9f, 0.9f, 0.9f, 0.6f);
		}
	}
