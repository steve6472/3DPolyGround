...
public class ImageButton extends Button
{
	...
	public ImageButton(EnumSizeType size, int x, int y, int dx, int dy)
	{
		this.size = size;
		this.X = x;
		this.Y = y;
	...
	public void render()
	{
		...
		if (enabled && hovered)
			SpriteRender.renderSpriteFromAtlas(x, y, size.bw, size.bh, 0, 3, 0, 4, 4);
	...
	public void setSize(int w, int h)
	{
		try
		{
			throw new IllegalAccessException("ImageButton has fixed size!");
