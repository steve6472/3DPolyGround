...
class BlockSaver
{
	...
	private static JSONObject cubeJson(Cube cube)
	{
		...
		if (!cube.isHitbox())
			json.put("isHitbox", cube.isHitbox());
		if (!cube.isCollisionBox())
			json.put("isCollisionBox", cube.isCollisionBox());
	...
	{
		if (!face.hasProperty(FaceRegistry.texture) || face.getProperty(FaceRegistry.texture).isBlank())
			return null;
