		...
		if (click.getAction() == KeyList.PRESS)
		{
			if (click.getButton() == KeyList.RMB)
			{
				...
				if (WaterRegistry.volumes[blockId] > 0)
				{
					...
					subChunk.setLiquidVolumeEfficiently(x + clickedOn.getXOffset() - subChunk.getX() * 16, y + clickedOn.getYOffset() - subChunk.getLayer() * 16, z + clickedOn.getZOffset() - subChunk.getZ() * 16, liquid + 1000.0);
				}
			}
		}
	}
