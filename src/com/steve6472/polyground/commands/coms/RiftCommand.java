...
import com.steve6472.polyground.commands.arguments.RiftArgument;
import com.steve6472.polyground.commands.arguments.Vec3fArgument;
		...
		dispatcher.register(
			literal("rift")
					...
					literal("start")
						.then(argument("name", string())
							.executes(c ->
							{
								if (rift == null)
								{
									rift = new Rift(getString(c, "name"), new Vector3f(0, 0, 0), new Vector3f(0, 0, 0), 0, 0,
										new RiftModel());
				...
				.then(
					literal("delete")
						.then(
							argument("rift", RiftArgument.rift())
								.executes(c -> {
				...
				.then(
					literal("set")
						.then(
							literal("cam")
								.then(
									argument("position", Vec3fArgument.vec3())
										.executes(c -> {
											...
											} else
											{
												...
												c.getSource().sendFeedback(String.format("Camera is now at %.2f, %.2f, %.2f",
									...
									argument("position", Vec3fArgument.vec3())
										.executes(c -> {
											...
											} else
											{
												...
												c.getSource().sendFeedback(String.format("Correction is now at %.2f, %.2f, %.2f",
													rift.getCorrection().x, rift.getCorrection().y, rift.getCorrection().z));
												return 1;
											}
