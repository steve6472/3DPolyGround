...
import com.steve6472.polyground.block.blockdata.MicroBlockData;
...
public class MicroBlock extends Block implements IBlockData
{
	...
	public void onClick(SubChunk subChunk, BlockData blockData, Player player, EnumFace clickedOn, MouseEvent click, int x, int y, int z)
	{
		...
		if ((click.getButton() == KeyList.RMB || click.getButton() == KeyList.LMB) && click.getAction() == KeyList.PRESS)
		{
			...
			if (CaveGame.itemInHand.getBlockToPlace() == null && CaveGame.itemInHand.getBlockToPlace() != Block.air) return;
	...
	public List<Cube> getCubes(int x, int y, int z)
	{
		...
		for (MicroBlockData.PalleteEntry value : mbd.getPallete().values())
		{
			for (int i = 0; i < value.getCount(); i++)
			{
				for (Cube m : value.getBlock().getBlockModel().getCubes())
				{
					...
					if (!c.isHitbox()) continue;

					if (Intersectionf.intersectRayAab(
						player.getX(), player.getY() + player.eyeHeight, player.getZ(),
						player.viewDir.x, player.viewDir.y, player.viewDir.z,
						c.getAabb().minX + x, c.getAabb().minY + y, c.getAabb().minZ + z,
						c.getAabb().maxX + x, c.getAabb().maxY + y, c.getAabb().maxZ + z, hr))
					{
						if (hr.x <= closest)
						{
							hit = true;
		...
		if (hit)
		{
			for (MicroBlockData.PalleteEntry value : mbd.getPallete().values())
			{
				if (hitResult.getZ() != value.getBlock().getId()) continue;

				for (int i = 0; i < value.getCount(); i++)
				{
					if (hitResult.getX() != i) continue;
