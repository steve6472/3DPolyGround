package com.steve6472.polyground.world.interaction;
...
public class HitPicker
{
	...
	public void tick(Player player, CaveGame pg)
	{
		...
		if (hit)
		{
			...
			if (Math.abs(hitResult.getPx() - (hitResult.getAabb().maxX + hitResult.getX())) <= epsion) hitResult.setFace(EnumFace.NORTH);
			if (Math.abs(hitResult.getPx() - (hitResult.getAabb().minX + hitResult.getX())) <= epsion) hitResult.setFace(EnumFace.SOUTH);
			if (Math.abs(hitResult.getPy() - (hitResult.getAabb().maxY + hitResult.getY())) <= epsion) hitResult.setFace(EnumFace.UP);
			if (Math.abs(hitResult.getPy() - (hitResult.getAabb().minY + hitResult.getY())) <= epsion) hitResult.setFace(EnumFace.DOWN);
			...
			for (Cube t : hitResult.getBlock().getCubes(hitResult.getX(), hitResult.getY(), hitResult.getZ()))
			{
				...
				AABBUtil.renderAABB(
					...
					1f, pg.basicTess, CaveGame.shaders.mainShader);
			}
